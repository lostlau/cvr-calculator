...
diab_type = CategoricalDtype(
    categories=['No_(0,6]', 'No_(6,10.5]', 'Yes_(0,7]', 'Yes_(7,8.5]', 'Yes_(8.5,10.5]', 'Gly_abn'], ordered=True)
...
def xvar_value2label(xvar):
    value_list = ['Gender', 'Age_group', 'Smoker', 'BMI_group', 'CHr_group', 'Diab_Gly_group', 'BP_group', 'Dis_Histo']
    label_list = ["Gender", "Age", "Smoker", "BMI", "Cholesterol-HDL ratio", "Diabetes & Glycohemoglobin",
                  "Blood Pressure", "Disease History"]
    return label_list[value_list.index(xvar)]


def logitreg_fit(data_surv, xvar_list, test=None):
    data_raz = pd.DataFrame(np.repeat(data_surv.values, np.ceil(data_surv.Surv / 12).astype("int64"), axis=0),
                            columns=data_surv.columns)
    FUY_list = []
    Death_list = []
    for seqn, group in data_raz.groupby("SEQN", sort=False):
        FUY_list = np.concatenate((FUY_list, np.arange(len(group)))).astype("int")
        Death_list = np.concatenate((Death_list, list(np.zeros(len(group) - 1)) + list(np.unique(group.Death)))).astype(
            "int")
    ...
    X_train.drop(["Gender_Female", "Age_group_[18,30]", "Smoker_Non Smoker", 'BMI_group_(18.5,33]', 'CHr_group_[0,6]',
                  'Diab_Gly_group_No_(0,6]',
    ...
    logitreg = sm.GLM(y_train, sm.add_constant(X_train, has_constant="add"), family=sm.families.Binomial())
    ...
    for ele in np.setdiff1d(X_ispl.columns, col_tmp):
        X_ispl[ele] = 0
    X_ispl.drop(["Gender_Female", "Age_group_[18,30]", "Smoker_Non Smoker", 'BMI_group_(18.5,33]', 'CHr_group_[0,6]',
                 'Diab_Gly_group_No_(0,6]',
                 'BP_group_Normal', 'Dis_Histo_No'], axis=1, inplace=True)
    qx_train = logitreg_fit.predict(sm.add_constant(X_ispl, has_constant="add"))
    ...
    if test is not None:
        ...
        for ele in np.setdiff1d(X_train.columns, col_tmp):
            X_test[ele] = 0
        X_test.drop(
            ["Gender_Female", "Age_group_[18,30]", "Smoker_Non Smoker", 'BMI_group_(18.5,33]', 'CHr_group_[0,6]',
             'Diab_Gly_group_No_(0,6]',
             'BP_group_Normal', 'Dis_Histo_No'], axis=1, inplace=True)
        qx_test = logitreg_fit.predict(sm.add_constant(X_test, has_constant="add"))
...
def poisreg_fit(data_surv, xvar_list, test=None):
    ...
    poisreg = sm.GLM(y_train, sm.add_constant(X_train, has_constant="add"), family=sm.families.Poisson())
    ...
    X_ispl.drop(["Gender_Female", "Age_group_[18,30]", "Smoker_Non Smoker", 'BMI_group_(18.5,33]', 'CHr_group_[0,6]',
                 'Diab_Gly_group_No_(0,6]',
                 'BP_group_Normal', 'Dis_Histo_No'], axis=1, inplace=True)
    qx_train = poisreg_fit.predict(sm.add_constant(X_ispl, has_constant="add"))
    e0_train = np.sum((1 - qx_train).values.reshape(-1, 17), axis=1)
    cindex_train = round(C_index(data_surv.Surv, e0_train, data_surv.Death), 5)
    ...
    if test is not None:
        ...
        X_test.drop(
            ["Gender_Female", "Age_group_[18,30]", "Smoker_Non Smoker", 'BMI_group_(18.5,33]', 'CHr_group_[0,6]',
             'Diab_Gly_group_No_(0,6]',
             'BP_group_Normal', 'Dis_Histo_No'], axis=1, inplace=True)
        qx_test = poisreg_fit.predict(sm.add_constant(X_test, has_constant="add"))
        e0_test = np.sum((1 - qx_test).values.reshape(-1, 17), axis=1)
        cindex_test = round(C_index(testdata.Surv, e0_test, testdata.Death), 5)
    else:
        cindex_test = np.nan
...
def death_proba(df, UW_year, xvar):
    with warnings.catch_warnings():
        ...
        freq_table = df[xvar][(df.Surv <= UW_year * 12) * (df.Death == 1)].value_counts(sort=False) / df[
            xvar].value_counts(sort=False)
...
def modeling_launch(values, data, percent):
    ...
    for mdl in values:
        if mdl == "Logistic Model":
            ...
            res_list.append({"Model": mdl, "Training": "not accessible", "Test": "not accessible"})
...
def mapping_casestudy(data_ref, xvar_list, age, gender, smoker, h, w, sbp, dbp, diab, gly, tc, hdl, diss):
    ...
    df_tmp = pd.DataFrame({"Gender": Gender, "Age_group": Age_group, "Smoker": Smoker, "BMI_group": BMI_group,
                           "CHr_group": CHr_group, "Diab_Gly_group": Diab_Gly_group, "BP_group": BP_group,
                           "Dis_Histo": Dis_Histo},
    ...
    for ele in np.setdiff1d(df_ref.columns, col_tmp):
        df_tmp[ele] = 0
    df_tmp.drop(["Gender_Female", "Age_group_[18,30]", "Smoker_Non Smoker", 'BMI_group_(18.5,33]', 'CHr_group_[0,6]',
                 'Diab_Gly_group_No_(0,6]',
                 'BP_group_Normal', 'Dis_Histo_No'], axis=1, inplace=True)
    return df_tmp
...
def casestudy_results(df, selected_model, max_FUY=17):
    ...
    qx = selected_model.predict(sm.add_constant(df, has_constant="add"))
...
final_model = {}
n_clickss = {"n_uploaddata": 0, "n_toydata": 0}
style_DivStep2 = {'width': "100%", "margin": "20px", "visibility": "hidden"}
style_DivStep3 = {'width': "100%", "margin": "20px", "visibility": "hidden"}
style_DivStep3_report = {'width': "100%", "margin": "10px", "visibility": "hidden"}
style_DivStep4 = {'width': "100%", "margin": "20px", "visibility": "hidden"}
style_DivStep4_survcurve = {'width': "70%", "marginTop": "30px", "paddingLeft": "12%", "paddingRight": "12%",
                            "visibility": "hidden"}
    ...
    return html.Div([
        ...
        html.Div(id='Div-Upload-Data',
                 children=[
                     ...
                     html.Div([html.Button("Upload!", id="Button-data-upload",
                               ...
                               html.Small(id="Text-data-upload",
                                          style={"display": "inline-block", "textAlign": "center", "color": "#20c307"})]),
                     # html.Br(),
                     html.H6("Or use directly the default toy data:"),
                     html.Div([html.Button("Use toy data", id="Button-toydata-use",
                                           style={"margin": "10px", 'textAlign': 'center', "display": "inline-block",
                                                  "backgroundColor": '#FCDA44'}),
                               html.Small(id="Text-toydata-use",
                                          style={"display": "inline-block", "textAlign": "center", "color": "#20c307"})]),
                     html.Br(),
        ...
        html.Div(id="Div-Descrpt-Stats",
                 children=[
                     ...
                     dcc.Dropdown(
                         id="xvar-dropdown",
                         options=[{"label": xvar_value2label(xvar), "value": xvar} for xvar in xvar_list],
                         value="Gender",
        ...
        html.Div(id='Div-Modeling',
                 children=[
                     ...
                     html.Div([
                         html.Header(html.Strong("Determine candidate models:"),
                         ...
                         dcc.Dropdown(
                             ...
                             options=[
                                 ...
                                 {"label": 'Gradient Boosting Machine', 'value': 'Gradient Boosting Machine'}
                             ],
                             value="",
                             multi=True,
                             style={'width': '90%', "margin": "5px"}
                         )], style={'width': '25%', "margin": "10px", "marginLeft": "15px", "display": "inline-block",
                                    'verticalAlign': 'middle'}),

                     html.Div(children=[
                         html.Header(html.Strong("Folds number of cross validation:"),
                                     style={"margin": "0px", "verticalAlign": "middle"}),
                         dcc.Input(
                             id="Input-CVfolds",
                     ...
                     html.Div(children=[
                         html.Header(html.Strong("Training/Test data split percent:"),
                                     style={"margin": "0px", "verticalAlign": "middle"}),
                         dcc.Slider(
                     ...
                     html.Div([
                         ...
                         html.Div([
                             ...
                             html.Div([html.Button("Submit", id="Button-model-determine",
                                                   style={"margin": "10px", "backgroundColor": '#FCDA44',
                                                          "display": "inline-block"}),
                                       html.Small(id="Text-model-determine",
        ...
        html.Div(id='Div-Case-Study',
                 children=[
                     ...
                     html.Div([
                         html.Div([
                             html.Div([
                                 html.Strong("Age (only 18-70 acceptable):", style={"margin": "5px", "display": "block"}),
                             ...
                             html.Div([
                                 ...
                                 dcc.Dropdown(id="Input-gender",
                                              options=[{"label": ele, "value": ele} for ele in ["Male", "Female"]],
                             ...
                             html.Div([
                                 ...
                                 dcc.Dropdown(id="Input-smoke",
                                              options=[{"label": ele, "value": ele} for ele in ["Smoker", "Non Smoker"]],
                                              value=None, style={"margin": "5px", "display": "block"})
                             ], style={"width": "30%", "marginLeft": "15px", "marginRight": "15px",
                                       "display": "inline-block",
                                       'verticalAlign': 'middle'}),
                         ...
                         html.Div([
                             html.Div([
                                 ...
                                 dcc.Input(id="Input-height", placeholder='Enter a number...', type='number', value=None,
                                           style={"width": "100%", "margin": "5px", "display": "block"}),
                             ], style={"width": "20%", "marginLeft": "28px", "marginRight": "28px",
                                       "display": "inline-block",
                                       'verticalAlign': 'middle'}),
                             html.Div([
                                 ...
                                 dcc.Input(id="Input-weight", placeholder='Enter a number...', type='number', value=None,
                                       ...
                                       'verticalAlign': 'middle'}),
                             html.Div([
                                 html.Strong("Systolic Blood Pressure (eg. 130):",
                                 ...
                                 dcc.Input(id="Input-sbp", placeholder='Enter a number...', type='number', value=None,
                                       ...
                                       'verticalAlign': 'middle'}),
                             html.Div([
                                 html.Strong("Diastolic Blood Pressure (eg. 80):",
                                             style={"margin": "5px", "display": "block"}),
                         ...
                         html.Div([
                             html.Div([
                                 ...
                                 dcc.Dropdown(id="Input-diab", options=[{"label": ele, "value": ele} for ele in
                                       ...
                                       'verticalAlign': 'middle'}),
                             html.Div([
                                 html.Strong("Glycohemoglobin(%; eg. 5):", style={"margin": "5px", "display": "block"}),
                                 dcc.Input(id="Input-glyco", placeholder='Enter a number...', type='number', value=None,
                                       ...
                                       'verticalAlign': 'middle'}),
                             html.Div([
                                 html.Strong("Total Cholesterol(mmol/L; eg. 5):",
                                             style={"margin": "5px", "display": "block"}),
                                 dcc.Input(id="Input-tc", placeholder='Enter a number...', type='number', value=None,
                                       ...
                                       'verticalAlign': 'middle'}),
                             html.Div([
                                 html.Strong("HDL-C(mmol/L; eg. 1):", style={"margin": "5px", "display": "block"}),
                         ...
                         html.Div([
                             ...
                             dcc.Checklist(id="Input-dishis",
                                           options=[{"label": ele, "value": ele} for ele in ["Heart Disease", "Stroke"]],
                             ...
                             html.Button("Calculate!", id="Button-calculator",
                                         style={"marginLeft": "50%", "backgroundColor": '#FCDA44',
                                                "display": "inline-block"})
...
@app.callback([Output('dashtable-data-upload', 'children'),
               ...
               Output("Text-toydata-use", "children"),
               Output("Text-data-upload", "style"),
              ...
              [Input("Button-data-upload", "n_clicks"),
               Input("Button-toydata-use", "n_clicks")],
              [State('upload-data', 'contents'),
               State('upload-data', 'filename')])
def update_loaddata(n_clicks1, n_clicks2, content, name):
    children_dashtable = ""
    children_text_upload = ""
    children_text_toy = ""
    style_text_upload = {"display": "inline-block", "textAlign": "center", "color": "#20c307"}
    if n_clickss["n_uploaddata"] != n_clicks1 and n_clicks1 != None and content is not None:
        ...
        children_text_upload = "External data uploaded successfully!"
        style_DivStep2.update({"display": "inline-block", "visibility": "visible"})
        style_DivStep3.update({"display": "inline-block", "visibility": "visible"})
    ...
    elif n_clickss["n_uploaddata"] != n_clicks1 and n_clicks1 != None and content == None:
        children_text_upload = "No data uploaded, please try again or use toy data!"
        style_text_upload.update({"color": "#f0160b"})
        style_DivStep2.update({"visibility": "hidden"})
        style_DivStep3.update({"visibility": "hidden"})

    elif n_clickss["n_toydata"] != n_clicks2 and n_clicks2 != None:
        df_tmp = data_init
        filename_tmp = "Toy Database"
        children_dashtable = tmpdata2dashtable(df_tmp, filename_tmp)
        children_text_toy = "Toy data extracted successfully!"
        style_DivStep2.update({"display": "inline-block", "visibility": "visible"})
        style_DivStep3.update({"display": "inline-block", "visibility": "visible"})
    ...
    n_clickss.update({"n_uploaddata": n_clicks1, "n_toydata": n_clicks2})

    return children_dashtable, children_text_upload, children_text_toy, style_text_upload, max_surv_year, marks_tmp, style_DivStep2, style_DivStep3
...
def update_figure(selected_xvar, selected_UWY):
    ...
    return {"data": [{
        ...
        "layout": {
            "title": "empirical mortality on different classes of " + xvar_value2label(selected_xvar),
            "xaxis": {"title": xvar_value2label(selected_xvar), "showgrid": False},
            "yaxis": {"title": "Frequence", "showgrid": False},
            "yaxis2": {"title": "Percentage", "overlaying": 'y', "side": 'right', "range": [0, y2_up],
                       "showgrid": False},
            'legend': dict(orientation='v', yanchor='top', xanchor='right', y=1.2, x=1.3)
        }
    }
...
def update_modeling_candidates(n_clicks, values, size):
    if n_clicks is None:
        ...
        style_DivStep3_report.update({"display": "block", "visibility": "visible"})
        ...
        ], style={"width": "50%", "margin": "10px", 'marginLeft': "25%", 'marginRight': "25%", "textAlign": "center",
                  "height": "200px"}
        )
...
@app.callback(
    [Output(component_id="Div-Case-Study", component_property="style"),
     Output("Text-model-determine", "children")],
...
def update_final_model(n_clicks, model_name):
    if n_clicks is None:
        ...
        try:
            ...
            text_model_determin = ""
            style_DivStep4.update({"display": "inline-block", "visibility": "visible"})
        except:
            text_model_determin = "This model is not accessible now, please change another!"
        return style_DivStep4, text_model_determin
...
def update_survivalcurve(n_clicks, age, gender, smoker, h, w, sbp, dbp, diab, gly, tc, hdl, diss):
    if any(ele is None for ele in [n_clicks, age, gender, smoker, h, w, sbp, dbp, diab, gly, tc, hdl]):
        ...
        style_DivStep4_survcurve.update({"display": "block", "visibility": "visible"})
...
def main():
    # logger.info("启动DashApp")
    # app.run_server(host="0.0.0.0", port=8089)
    WSGIServer(('0.0.0.0',8069), app.server).serve_forever()
